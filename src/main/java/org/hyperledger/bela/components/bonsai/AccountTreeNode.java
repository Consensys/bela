...
public class AccountTreeNode extends AbstractBonsaiNode {
    ...
    public AccountTreeNode(final BonsaiStorageView bonsaiStorageView, final Node<Bytes> node) {
        super(calculateLabel(node));
                ...
                .map(node -> {
                    if (bonsaiStorageView.nodeIsHashReferencedDescendant(this.node, node)) {
                        ...
                        return new AccountValueNode(bonsaiStorageView, accountHash, node.getValue()
                                .get(), AccountValueNode.StorageLocation.BONSAI);

                    } else {
                        return new LabelNode("Missing value for ", calculateLabel(node));
                    }
